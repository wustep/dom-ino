...
import { bodyPositionsChanged, type BodyPos } from "./physics";
...
describe("bodyPositionsChanged", () => {
  ...
  it("handles multiple bodies correctly", () => {
    const prev = mapOf([["a", pos(10, 20)], ["b", pos(30, 40)]]);
    const next = mapOf([["a", pos(10, 20)], ["b", pos(30, 40)]]);
    expect(bodyPositionsChanged(prev, next)).toBe(false);

    const moved = mapOf([["a", pos(10, 20)], ["b", pos(31, 40)]]);

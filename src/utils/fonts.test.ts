...
describe("buildFontString", () => {
  ...
  it("includes font style when not normal", () => {
    expect(buildFontString(16, 400, undefined, "italic")).toBe(
      `italic 16px ${DEFAULT_SERIF}`
    );
  });
  ...
  it("uses custom font family", () => {
    expect(buildFontString(18, undefined, "Arial, sans-serif")).toBe(
      "18px Arial, sans-serif"
    );
  });

  it("omits style prefix when normal", () => {
    expect(buildFontString(16, 600, undefined, "normal")).toBe(
      `600 16px ${DEFAULT_SERIF}`
    );
  });
});
...
describe("parseFontShorthand", () => {
  ...
  it("parses font with style and weight", () => {
    expect(
      parseFontShorthand('italic 700 17px "Source Serif 4", Georgia, serif')
    ).toEqual({
      style: "italic",
      weight: 700,
      family: '"Source Serif 4", Georgia, serif',

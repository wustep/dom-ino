...
describe("toAbsoluteUrl", () => {
	...
	it("preserves data: URLs", () => {
		expect(toAbsoluteUrl("data:image/png;base64,abc", BASE)).toBe("data:image/png;base64,abc")
	})

	it("resolves relative paths against base URL", () => {
		expect(toAbsoluteUrl("../images/photo.jpg", BASE)).toBe("https://example.com/images/photo.jpg")
	})

	it("resolves root-relative paths", () => {
		expect(toAbsoluteUrl("/static/logo.svg", BASE)).toBe("https://example.com/static/logo.svg")
	})

	it("resolves same-directory paths", () => {
		expect(toAbsoluteUrl("thumb.png", BASE)).toBe("https://example.com/page/thumb.png")
	})

	it("trims whitespace", () => {
		expect(toAbsoluteUrl("  /img.png  ", BASE)).toBe("https://example.com/img.png")
	})

	it("handles mailto: scheme", () => {
		expect(toAbsoluteUrl("mailto:user@example.com", BASE)).toBe("mailto:user@example.com")
	})
})
...
describe("toAbsoluteSrcset", () => {
	...
	it("resolves relative URLs in srcset entries", () => {
		...
		expect(result).toBe("https://example.com/img-sm.jpg 320w, https://example.com/img-lg.jpg 1024w")
	})

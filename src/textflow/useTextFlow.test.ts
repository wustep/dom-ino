...
function makeLine(text: string, width: number, start: LayoutCursor, endGrapheme: number): LayoutLine {
  return {
    text,
    width,
    start,
...
describe("computeTextFlow", () => {
  ...
  it("skips segments that break words when allowWordBreaks is false", () => {
    ...
    const result = computeTextFlow(
      "Hello world", FONT, 20, 0, 0, 400, 200, [], 8, 8,
  ...
  it("starts layout from a provided startCursor", () => {
    ...
    const result = computeTextFlow(
      "Some text. continued text", FONT, 20, 0, 0, 400, 200, [],
      8, 8, true, start
    );

	...
	return (
		<>
			...
			{openPanel === "pages" && (
				...
				>
					<div
						style={{ padding: "12px 14px 8px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}
					>
						...
						<div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
							...
							{customPages
								.filter(
									(cp) =>
										cp.kind !== "snapshot" ||
										!WEBSITE_PRESETS.some((wp) => wp.url === cp.sourceUrl),
								)
								.map((cp) => (
									<button
										key={cp.id}
			...
			>
				...
				>
					...
					>
						<StashIcon />
						{savedElements.length > 0 && (
							<span style={{ fontSize: 8, fontWeight: 700, color: "#a78bfa", marginLeft: -2 }}>
...
function Sep() {
	return (
		<div
			style={{ width: 1, height: 16, backgroundColor: "rgba(255,255,255,0.06)", margin: "0 1px" }}
		/>
	)
}
...
function StashIcon() {
	return (
		<svg
			...
			strokeLinejoin="round"
		>
			<path d="M2 4.5L7 1.5L12 4.5L7 7.5Z" />
			<path d="M2 7L7 10L12 7" />
			<path d="M2 9.5L7 12.5L12 9.5" />
		</svg>
	)
}

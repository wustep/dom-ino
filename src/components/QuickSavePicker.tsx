...
export const QuickSavePicker = memo(function QuickSavePicker({
  ...
  const handleSave = useCallback((candidate: SavePickerCandidate) => {
    ...
    <>
      ...
      {candidates.map((candidate) => {
        ...
        return (
          <div
            key={candidate.id}
            ref={(el) => { if (el) candidateRefs.current.set(candidate.id, el); else candidateRefs.current.delete(candidate.id); }}
          ...
          >
            ...
            <SaveBadge
              ...
              wide={wide}
              isHovered={hovered}
              onClick={saved && onUnsave ? (e) => { e.stopPropagation(); onUnsave(candidate.id); } : undefined}
              style={{
                position: "absolute",
                left: PICKER_HALO - 8,
                top: PICKER_HALO - 10,

	...
	it("captures anchor color runs when the text block is temporarily revealed", () => {
		...
		const getComputedStyleSpy = vi.spyOn(window, "getComputedStyle").mockImplementation((el) => {
			...
			const inheritedVisibility =
				paragraph.style.visibility === "hidden" && el instanceof HTMLElement && el !== paragraph
					? "hidden"
					: style.visibility
...
describe("inferSnapshotElementType", () => {
	...
	it("returns paragraph for P tags", () => {
		...
		expect(inferSnapshotElementType(el, mockComputedStyle(), "Some text", defaultRect)).toBe(
			"paragraph",
		)
	})
	...
	it("returns link for inline A tags", () => {
		...
		expect(
			inferSnapshotElementType(el, mockComputedStyle({ display: "inline" }), "", defaultRect),
		).toBe("link")
	})
	...
	it("returns button for block A tags", () => {
		const el = makeElement("A")
		expect(
			inferSnapshotElementType(el, mockComputedStyle({ display: "block" }), "", defaultRect),
		).toBe("button")
	})
	...
	it("returns paragraph for generic div with long text and inline children", () => {
		...
		expect(
			inferSnapshotElementType(
				el,
				mockComputedStyle(),
				"Some long paragraph text content here",
				defaultRect,
			),
		).toBe("paragraph")
	})
	...
	it("returns card for elements with background color", () => {
		...
		expect(
			inferSnapshotElementType(
				el,
				mockComputedStyle({ backgroundColor: "rgb(255, 255, 255)" }),
				"",
				defaultRect,
			),
		).toBe("card")
	...
	it("returns badge for small elements with short text and background", () => {
		...
		expect(
			inferSnapshotElementType(
				el,
				mockComputedStyle({ backgroundColor: "rgb(0, 123, 255)" }),
				"New",
				defaultRect,
			),
		).toBe("badge")

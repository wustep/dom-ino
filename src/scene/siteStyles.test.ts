...
describe("siteStyles", () => {
	...
	it("forces the Wikipedia featured article summary container into imported text flow", () => {
		...
		expect(
			isPretextBlockEligible(node, makeTextElement(), "https://en.wikipedia.org/wiki/Main_Page"),
		).toBe(true)
	})
	...
	it("keeps Wikipedia page chrome out of automatic throwable selection", () => {
		...
		expect(isAutoSelectEligible(link, "https://en.wikipedia.org/wiki/History_of_art")).toBe(false)
	})
	...
	it("still auto-selects normal Wikipedia article media", () => {
		...
		expect(isAutoSelectEligible(image, "https://en.wikipedia.org/wiki/History_of_art")).toBe(true)
	})
	...
	it("force-selects the Wikipedia logo and search box", () => {
		...
		expect(isAutoSelectEligible(logo, "https://en.wikipedia.org/wiki/Main_Page")).toBe(true)
		expect(isAutoSelectEligible(search, "https://en.wikipedia.org/wiki/Main_Page")).toBe(true)
	})
	...
	it("force-selects descendants inside the Wikipedia search and page tool areas", () => {
		...
		expect(isAutoSelectEligible(input, "https://en.wikipedia.org/wiki/Main_Page")).toBe(true)
		expect(isAutoSelectEligible(link, "https://en.wikipedia.org/wiki/Main_Page")).toBe(true)
	})
	...
	it("preserves generic non-Wikipedia text blocks", () => {
		...
		expect(isPretextBlockEligible(node, makeTextElement(), "https://example.com/article")).toBe(
			true,
		)
	})
})

...
/**
 * Snapshot HTML using a hidden iframe so external stylesheets and
 * inline styles render properly. The iframe's srcdoc loads the full
 * HTML document, then we walk its body after it renders.
 */
export async function snapshotHtmlToScene(
  html: string, containerWidth: number = 1200, sceneName: string = "Custom Page"
): Promise<SceneDescription> {
  return new Promise((resolve) => {
    const iframe = document.createElement("iframe");
    iframe.style.cssText = `position:fixed;left:-10000px;top:0;width:${containerWidth}px;height:2000px;border:none;visibility:hidden;pointer-events:none;`;
    ...
    iframe.onload = () => {
      ...
      setTimeout(() => {
        try {
          ...
          walkElement(doc.body, elements, bodyRect, 0, doc.defaultView!);
...
/**
 * If iframe approach fails, create a basic scene from the raw text content.
 */
function makeFallbackScene(html: string, width: number, name: string): SceneDescription {
  const tmp = document.createElement("div");
  tmp.innerHTML = html;
  const text = tmp.textContent?.trim().slice(0, 2000) || "Could not parse page content.";

  return {
    id: `snapshot-${Date.now()}`, name, width, height: 800, backgroundColor: "#ffffff",
    elements: [{
      id: `snap-fallback-${snapshotCounter++}`, type: "paragraph",
      rect: { x: 40, y: 40, width: Math.min(width - 80, 700), height: 600 },
      throwable: false, pinned: true, text,
      fontSize: 16, fontWeight: 400, fontFamily: '"DM Sans", sans-serif',
      lineHeight: 26, color: "#333",
    }],
  };
}
...
function walkElement(el: HTMLElement, out: SceneElement[], rootRect: DOMRect, depth: number, win: Window) {
  ...
  for (const child of children) {
    ...
    if (tag === "SCRIPT" || tag === "STYLE" || tag === "NOSCRIPT" || tag === "SVG" || tag === "LINK" || tag === "META" || tag === "HEAD") continue;
    ...
    if (w < 4 || h < 4) { walkElement(child, out, rootRect, depth + 1, win); continue; }
